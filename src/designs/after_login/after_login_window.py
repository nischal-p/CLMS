...
import re
from designs.second_window.search_table_window import SearchTableWindow
...
class AfterLoginWindow(object):
    ...
    def __init__(self, main_window, staff_username):
        ...
        self.return_book_button = QtGui.QPushButton(self.borrow_tab)
        self.return_book_button.setGeometry(QtCore.QRect(190, 510, 181, 41))
        self.return_book_button.setObjectName(_fromUtf8("return_book_button"))

        self.flag_lost_button = QtGui.QPushButton(self.borrow_tab)
        self.flag_lost_button.setGeometry(QtCore.QRect(390, 510, 181, 41))
        self.flag_lost_button.setObjectName(_fromUtf8("flag_lost_button"))
        ...
        self.borrow_id_box = QtGui.QLineEdit(self.borrow_tab)
        self.borrow_id_box.setGeometry(QtCore.QRect(30, 520, 141, 31))
        self.borrow_id_box.setObjectName(_fromUtf8("borrow_id_box"))

        self.borrow_id_label = QtGui.QLabel(self.borrow_tab)
        self.borrow_id_label.setGeometry(QtCore.QRect(30, 500, 141, 21))
        self.borrow_id_label.setObjectName(_fromUtf8("borrow_id_label"))
        ...
        self.generate_reports_button = QtGui.QPushButton(self.reports_tab)
        self.generate_reports_button.setGeometry(QtCore.QRect(300, 30, 191, 41))
        self.generate_reports_button.setObjectName(_fromUtf8("generate_reports_button"))
    ...
    def retranslate_ui(self, main_window):
        main_window.setWindowTitle(_translate("main_window1", "Claremont LBMS", None))
        ...
        item.setText(_translate("main_window1", "Borrow ID", None))
        item = self.borrows_table.horizontalHeaderItem(1)
        item.setText(_translate("main_window1", "Book Title", None))
        item = self.borrows_table.horizontalHeaderItem(2)
        item.setText(_translate("main_window1", "Borrowed by", None))
        item = self.borrows_table.horizontalHeaderItem(3)
        item.setText(_translate("main_window1", "Issued by", None))
        item = self.borrows_table.horizontalHeaderItem(4)
        item.setText(_translate("main_window1", "Borrow Date", None))
        item = self.borrows_table.horizontalHeaderItem(5)
        item.setText(_translate("main_window1", "Due Date", None))
        item = self.borrows_table.horizontalHeaderItem(6)
        item.setText(_translate("main_window1", "Status", None))
        item = self.borrows_table.horizontalHeaderItem(7)
        item.setText(_translate("main_window1", "Return Date", None))
        item = self.borrows_table.horizontalHeaderItem(8)
        item.setText(_translate("main_window1", "Return Validated by", None))
        self.borrow_table_label.setText(_translate("main_window1",
                                                   "Select a record directly or search for a book using it\'s book title or borrower\'s username:",
                                                   None))
        self.return_book_button.setText(_translate("main_window1", "Return the book in this record", None))
        self.flag_lost_button.setText(_translate("main_window1", "Flag the book in this record as lost", None))
        ...
        self.borrow_id_label.setText(_translate("main_window1", "Enter borrowID:", None))
        ...
        self.generate_reports_button.setText(_translate("main_window1", "Generate Reports", None))
        ...
        item.setText(_translate("main_window1", "Number of times borrowed", None))
        ...
        self.book_reports_label.setText(_translate("main_window1", "Books borrowed the highest number of times:", None))
    ...
    def show_students_table(self):
        result = StudentDAO.show_all_students()
        self.students_table.setRowCount(0)
    ...
    def show_staff_table(self):
        result = StaffDAO.show_all_staff()
        self.staff_table.setRowCount(0)
        for row_number, row_data in enumerate(result):
    ...
    def book_extract_data_clicked(self):
        ...
        if is_isbn:
            ...
            book_title = BookDAO.check_book_in_database(isbn1)
            if book_title is not None:  # if book is already in database:
    ...
    def book_insert_clicked(self):
        ...
        authors = self.clean_string_input(authors)  # to clean the individual names
        if authors is None:
    ...
    def student_insert_clicked(self):
        ...
        email = email.replace('@claremontschool.co.uk', '')
        if '@' in email:
    ...
    def staff_insert_clicked(self):
        ...
        if len(password1) < 8:
            ...
            return
        hashed_password = PasswordOperations.get_hash(password1)
        unclean_strings = [fname, surname, email]
    ...
    def lend_book_clicked(self):
        ...
        borrower_id = self.borrow_username_box.text()
        book_title = BookDAO.check_book_in_database(book_id)
        ...
        student_name = StudentDAO.check_student_in_database(borrower_id)
        staff_name = StaffDAO.check_staff_in_database(borrower_id)
    ...
    def clean_string_input(self, strings):
        ...
        for i in strings:
            ...
            if i[0] == ' ':  # removes the first white space (if it's there)
                i[0] = ''
            if len(i) == 0:  # this applies to empty white space inputs
                return None
...
if __name__ == "__main__":
    ...
    window1 = AfterLoginWindow(main_window2, 'ram.lamichhane')
    main_window2.show()
    sys.exit(app.exec_())
